...
public class O_Bar_Dotted : MonoBehaviour
{
    ...
    public void OnValueDecrease()
    {
        ...
        else ML_Scale.Pop(1, 0.8f, 1.1f, 0, transform.Find("Group").GetChild(value_Current), 0.5f);

        if (value_Current <= 0) M_Game.instance.EnterResultChecking();
    }
    ...
    public void OnValueIncrease()
    {
        ...
        else ML_Scale.Pop(0, 1.2f, 0.9f, 1, transform.Find("Group").GetChild(value_Current - 1), 0.5f);
    }
